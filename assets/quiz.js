/* quiz.js — แบบฝึกที่ให้ feedback ทันที
   ...
   ⚠️ ตัวนับคะแนน — วาง <div id="score"></div> ไว้ที่ไหนก็ได้ในหน้า
   แล้วมันจะอัปเดตเอง พร้อมปุ่ม "📋 คัดลอกผล" + "↻ เริ่มใหม่" เมื่อทำครบทุกข้อ
   **บังคับสำหรับบทที่มีแบบฝึก** (เดิมเขียนว่า "ไม่บังคับ" — ไม่จริง)
   ปุ่มคัดลอกผลอยู่ใน board() ที่เดียว ⇒ ไม่มี element นี้ = ผู้เรียนส่งผลกลับมาไม่ได้
   และหน้ายังใช้ได้ปกติ ไม่มี error อะไรฟ้อง · check_lessons.py + check_layout.py
   บังคับข้อนี้แล้ว (เกิดจริงในบทที่ 9 · บทที่ 1 กับ _template.html ก็ขาดมาตลอด)
...
(function () {
  ...
  function norm(v) {
    return String(v).trim().toLowerCase()
      ...
      // ⁰¹²³… → เลขธรรมดา · คนก๊อปสมการที่เรนเดอร์แล้วจะได้ตัวยกแบบนี้
      .replace(/[\u2070\u00b9\u00b2\u00b3\u2074-\u2079]/g,
               function (c) { return '0123456789'['⁰¹²³⁴⁵⁶⁷⁸⁹'.indexOf(c)]; })
      .replace(/\*\*/g, '^')
      .replace(/[\s*·]/g, '')          // ช่องว่างและเครื่องหมายคูณไม่สำคัญ
      // ⚠️ ^ ไม่สำคัญ — ก๊อป $m^2$ จากหน้าที่เรนเดอร์แล้วจะได้ "m 2" (ไม่มี ^)
      //    และ "m²" ก็มาเป็นตัวยก ⇒ ไม่ตัดทิ้งจะ **ปฏิเสธคำตอบที่ถูก**
      //    (เกิดจริง 2026-09-24: ตอบ "pt · ( ptt − pxx + m 2 phi )" ถูกแต่ไม่ผ่าน)
      //    ตรวจแล้วว่าไม่ชนกับคีย์ไหนในทุกบท และไม่มีคีย์ไหนกลายเป็นตัวเลข
      .replace(/\^/g, '')
      .replace(/[()]/g, '');
  }
